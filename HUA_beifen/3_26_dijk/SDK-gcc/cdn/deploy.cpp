...
void solve()
{
    ZKW::zkw.costEdge = G_costEdge;
    ZKW::zkw.costNodeNum = G_costNodeNum;
    ZKW::zkw.init_p(G_netNodeNum, G_costNodeNum);
...
void parse_file(char* topo[MAX_EDGE_NUM], int line_num) {
    ...
    Dijkstra::dijk.init(G_netNodeNum);
    ...
    for(int i=0; i<G_netEdgeNum; ++i) {
        ...
        Dijkstra::dijk.AddEdge(start, end, cost);
        Dijkstra::dijk.AddEdge(end, start, cost);
    }
    ++line_id;
