...
void ZKW::change_work(bool* video_list)
{
    for(int i=head[src]; i!=-1; i=edge[i].next)
    {
        ...
        if(edge[i].cost == 0 && !video_list[v])      //之前存在现在不存在
        {
            ...
            ans_cost += (edge[i].pre_cap - edge[i].cap)*MAX_COST;
        }
    ...
    for(int i=this->src; i>=0; i--){
        do {
            memset(vis, 0, sizeof(vis));
        }while(change_dfs(i, false, i, INF, 0, 0));
        printf("cost=%d flow=%d\n", ans_cost, ans_flow);
    }
}

int ZKW::change_dfs(int st, bool flag, int now_node, int max_f, int sum_cost, int dep)
{
//    printf("-=- %d  %d  %d\n", st, now_node, dep);
//    if(now_node == st && flag) {
////        printf("%d   %d\n", sum_cost, ans_flow);
//
//        if(sum_cost >= 0 || max_f == 0)
//            return 0;
//        ans_flow += max_f;
//        return max_f;
//    }
    ...
    for(int i=head[now_node]; i != -1; i=edge[i].next){
        if(edge[i].cap &&  !vis[edge[i].v]){
            ...
            int f = 0;
            if(edge[i].v == st){
                if(sum_cost + edge[i].cost < 0) {
                    f = std::min(edge[i].cap, max_f - used);
                    ans_flow += f;
                }
            }else {
                f = change_dfs(st, true, edge[i].v, std::min(edge[i].cap, max_f - used), sum_cost + edge[i].cost,
                               dep + 1);
            }
