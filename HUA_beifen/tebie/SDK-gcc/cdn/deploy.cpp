...
void tsolve()
{
    ...
    for(int i=0; i<G_costNodeNum; i++)
        ans[G_costEdge[i].node_id] = G_costEdge[i].flow_need;

    v_cost = G_netNodeNum*G_videoCost;
    ...
    while(1) {
        ...
        for(int i=0; i<G_netNodeNum; i++) {
            if (ans[i]) {
                int to = -1;
                int cost = INF;
                for (int k = G_head[i]; k != -1; k = G_edge[k].next) {
                    if (k % 2 != 0 || G_edge[k].cap < ans[i])
                        continue;
                    int v = G_edge[k].v;
                    if (G_edge[k].cost != 0 && cost > G_edge[k].cost) {
                        cost = G_edge[k].cost;
                        to = v;
                    }
                }
        ...
        ZKW::zkw.init(G_edge, G_edgeNum, G_netNodeNum + 2, G_head, G_netNodeNum, G_netNodeNum + 1, v);
        ZKW::zkw.costflow();

//        printf("%d %d\n", ZKW::zkw.ans_flow, ZKW::zkw.ans_cost);

        if (ZKW::zkw.ans_flow == G_sumFlow) {
            if (ZKW::zkw.ans_cost != 0) {
                int num = 0;
                for (int i = 0; i < G_netNodeNum; i++) {
                    if (v[i])
                        ++num;
                }
                if(ZKW::zkw.ans_cost + num * G_videoCost < v_cost) {
                    memcpy(tttt, v, sizeof(tttt));
                    v_cost = ZKW::zkw.ans_cost + num * G_videoCost;
                    ooo = 0;
                }
//                printf("-=-= %d -=-=\n", v_cost);
...
//    ZKW::zkw.init(G_edge, G_edgeNum, G_netNodeNum + 2, G_head, G_netNodeNum, G_netNodeNum + 1, v);
//    for(int i=0; i<G_netNodeNum; i++)
//        ZKW::zkw.add_edge(ZKW::zkw.src, i, INF, G_videoCost*1.0/G_capSum[i]);
//    ZKW::zkw.costflow();
//
//    int num = 0;
//    for(int i=ZKW::zkw.head[ZKW::zkw.src]; i!=-1; i=ZKW::zkw.edge[i].next)
//    {
//        int t = ZKW::zkw.edge[i].v;
//        if(ZKW::zkw.edge[i].cap != ZKW::zkw.edge[i].pre_cap) {
//            v[t] = true;
//            num++;
//        }
//    }
...
//
//
//    printf("%d %d\n", ZKW::zkw.ans_cost+num*G_videoCost, ZKW::zkw.ans_flow);
//
//    return;
...
bool f[NODE_MAXNUM][NODE_MAXNUM];
int t[NODE_MAXNUM];
...
//        t[start]++;
//        t[end]++;
//        f[start][end] = f[end][start] = true;
    }
    ++line_id;

//    PageRank::pageRank.init(G_netNodeNum, t, f);
//    PageRank::pageRank.work();
